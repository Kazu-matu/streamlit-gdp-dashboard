"""
views/local_csv_tab.py — ローカル CSV データの可視化ビュー。
"""
...
def render_local_csv(df: pd.DataFrame) -> None:
    """CSVデータダッシュボードを描画する。
    ...
    st.markdown(f"#### 📊 CSV 主要指標（{latest_year}年）")
    ...
    # CSV は 積層エリアチャート (Area Chart) を使用して可視化のバリエーションを表現
    fig = px.area(
        df,
        x="年", y="GDP (USD)", color="国",
        title="ローカル CSV データ GDP 推移 (エリアグラフ)",
    ...
    data_table_with_download(formatted, "local_csv_gdp.csv")
