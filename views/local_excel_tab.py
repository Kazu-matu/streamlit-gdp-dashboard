"""
views/local_excel_tab.py — ローカル Excel データの可視化ビュー。
"""
...
def render_local_excel(df: pd.DataFrame) -> None:
    """Excelデータダッシュボードを描画する。
    ...
    st.markdown(f"#### 📊 Excel 主要指標（{latest_year}年）")
    ...
    # Excel は折れ線グラフ (Line Chart) で可視化
    fig = px.line(
        df,
        x="年", y="GDP (USD)", color="国",
        title="ローカル Excel データ GDP 推移 (折れ線グラフ)",
    ...
    fig.update_traces(line=dict(width=3, dash="dashdot"))
    ...
    data_table_with_download(formatted, "local_excel_gdp.csv")
